package Sphere::Controller::Directory::Posts;
...
use Sphere::Form::Directory::Post;

=head1 NAME

Sphere::Controller::Directory::Posts - Catalyst Controller
...
sub base : Chained('../directory') PathPart('posts') CaptureArgs(0) {
    my ( $self, $c ) = @_;
...
sub add : Chained('base') PathPart('add') Args(0) {
	...
	my $form = Sphere::Form::Directory::Post->new;
...
sub form : Private {
    ...
    if ($c->stash->{entry}) {
	$c->stash( template => 'directory/posts/edit.tt' );
    } else {
	$c->stash( template => 'directory/posts/add.tt' );
    }
}
